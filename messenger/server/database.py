...
#
#
# #  Класс для работы с БД сервера. Использует SQLite БД, реализован с помощью SQLAlchemy ORM (классический подход.)
# class ServerStorage:
...
#         self.metadata.create_all(self.database_engine)
#
#         # Создаем отображения. Связываем класс ORM с таблицей.
#         mapper(self.AllUsers, users_table)
#         mapper(self.ActiveUsers, active_users_table)
#         mapper(self.LoginHistory, user_login_history)
#         mapper(self.UsersContacts, contacts)
...
#
#         # Если имя пользователя уже присутствует в таблице, обновляем время последнего входа.
#         # Проверяем корректность ключа. Если клиент прислал новый ключ, сохраняем его.
#         if rez.count():
#             user = rez.first()
#             user.last_login = datetime.datetime.now()
#             if user.pubkey != key:
#                 user.pubkey = key
#         # Если такого пользователя нет, то генерируем исключение.
#         else:
#             raise ValueError('Пользователь не зарегистрирован.')
#
...
#         self.session.commit()
#
#     # Функция получения хэша пароля пользователя.
#     def get_hash(self, name):
#         user = self.session.query(self.AllUsers).filter_by(name=name).first()
#         return user.passwd_hash
#
...
#         return user.pubkey
#
#     # Функция проверяющая существование пользователя.
#     def check_user(self, name):
#         if self.session.query(self.AllUsers).filter_by(name=name).count():
#             return True
#         else:
...
#     def user_logout(self, username):
#         # Запрашиваем пользователя который выходит из таблицы AllUsers
